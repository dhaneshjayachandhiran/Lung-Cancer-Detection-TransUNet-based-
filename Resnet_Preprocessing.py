...
class Config:
    ...
    # This is the path the model is looking for
    OUTPUT_DIR = os.path.join(ROOT_DIR, 'ResNet_Preprocessed_Data')
...
if __name__ == '__main__':
    ...
    for subset_path in glob(os.path.join(Config.SUBSETS_DIR, "subset*")):
        ...
        # Model expects: ResNet_Preprocessed_Data/resnet_pre_subsetX/images/
        img_out = os.path.join(Config.OUTPUT_DIR, f'resnet_pre_{subset_name}', 'images')
        mask_out = os.path.join(Config.OUTPUT_DIR, f'resnet_pre_{subset_name}', 'masks')
        ...
        for _, row in tqdm(final_df.iterrows(), total=len(final_df), desc=f"Preprocessing {subset_name}"):
            ...
            # Create/Save Mask (Middle Slice)
            mask = np.zeros((64, 64), dtype=np.float32)
            if row['class'] == 1:
                # Simple circle mask for the middle slice
                mask_3d = create_spherical_mask((64,64,64), (32,32,32), 5) # Default 5mm radius for mask
                mask = mask_3d[32, :, :].astype(np.float32)
            np.save(os.path.join(mask_out, fname), mask)

    print("✅ Preprocessing Done! Now run Resnet_Model.py")